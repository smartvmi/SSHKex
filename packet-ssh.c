...
struct ssh_key_list {
    const gchar *iv_client_to_server;
    const gchar *iv_server_to_client;
    const gchar *key_client_to_server;
    const gchar *key_server_to_client;
    const gchar *enc_name;
};

static struct ssh_key_list ssh_keys;
...
static StringInfo ssh_decrypted_data = {NULL, 0};
...
//    ContentType type;       /**< Content type of the decrypted record data. */
    ...
    struct _SshRecordInfo* next;
} SshRecordInfo;

typedef struct {
    SshRecordInfo *records; /**< Decrypted records within this frame. */
...
static void ssh_save_decrypted_record(packet_info *pinfo, gint record_id, guint8 content_type, guint8 curr_layer_num_ssh);
void ssh_add_record_info(gint proto, packet_info *pinfo, const guchar *data, gint data_len, gint record_id, guint8 curr_layer_num_ssh);
SshPacketInfo * ssh_add_packet_info(gint proto, packet_info *pinfo, guint8 curr_layer_num_ssh);
tvbuff_t * ssh_get_record_info(tvbuff_t *parent_tvb, int proto, packet_info *pinfo, gint record_id, guint8 curr_layer_num_ssh,
                    SshRecordInfo **matched_record);

static void
ssh_save_decrypted_record(packet_info *pinfo, gint record_id, guint8 content_type, guint8 curr_layer_num_ssh) {
    const guchar *data = ssh_decrypted_data.data;
    guint datalen = ssh_decrypted_data.data_len;
    ...
    ssh_add_record_info(proto_ssh, pinfo, data, datalen, record_id, curr_layer_num_ssh);
}

void
ssh_add_record_info(gint proto, packet_info *pinfo, const guchar *data, gint data_len, gint record_id, guint8 curr_layer_num_ssh) {
    SshRecordInfo *rec, **prec;
    SshPacketInfo *pi = ssh_add_packet_info(proto, pinfo, curr_layer_num_ssh);
...
/* Links SSH records with the real packet data. {{{ */
SshPacketInfo *
ssh_add_packet_info(gint proto, packet_info *pinfo, guint8 curr_layer_num_ssh)
{
    SshPacketInfo *pi = (SshPacketInfo *)p_get_proto_data(wmem_file_scope(), pinfo, proto, curr_layer_num_ssh);
    if (!pi) {
        pi = wmem_new0(wmem_file_scope(), SshPacketInfo);
        pi->srcport = pinfo->srcport;
        pi->destport = pinfo->destport;
        p_add_proto_data(wmem_file_scope(), pinfo, proto, curr_layer_num_ssh, pi);
    }
...
tvbuff_t *
ssh_get_record_info(tvbuff_t *parent_tvb, int proto, packet_info *pinfo, gint record_id, guint8 curr_layer_num_ssh,
                    SshRecordInfo **matched_record) {
    SshRecordInfo *rec;
    SshPacketInfo *pi;
    pi = (SshPacketInfo *) p_get_proto_data(wmem_file_scope(), pinfo, proto, curr_layer_num_ssh);
...
dissect_ssh(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree, void *data _U_) {
    ...
    while (tvb_reported_length_remaining(tvb, offset) > 0) {
        ...
        int i;
        for (i = 0; i < length; ++i) {
            sprintf(&converted[i * 2], "%02X", target[i]);
        }
        ...
        //----------------------------------------!! Decryption Python starts here!!---------------------------------
        // Python function call
        PyObject *pName, *pModule, *pFunc1, *pFunc2, *pResult, *pValue1;

        Py_Initialize();
        PyRun_SimpleString("import sys");
        PyRun_SimpleString("import os");
        PyRun_SimpleString("import binascii");
        PyRun_SimpleString("from Cryptodome import Random");
        PyRun_SimpleString("from Cryptodome.Cipher import AES");
        PyRun_SimpleString("from Cryptodome.Cipher import ChaCha20");

        pName = PyUnicode_FromString("read_files_for_wireshark");
        ...
        if (pModule != NULL) {
            pFunc1 = PyObject_GetAttrString(pModule, (char *) "read_files");
            pFunc2 = PyObject_GetAttrString(pModule, (char *) "start_decryption");

            if (!(pFunc1 && pFunc2)) {
                return -1;
            }
            if (!(PyCallable_Check(pFunc1) && PyCallable_Check(pFunc2))) {
                return -1;
            }

            pValue1 = Py_BuildValue("(ssss)", (char *) ssh_keys.iv_client_to_server,
                                    (char *) ssh_keys.iv_server_to_client, (char *) ssh_keys.key_client_to_server,
                                    (char *) ssh_keys.key_server_to_client);
            PyObject *pValue2 = Py_BuildValue("(ssi)", (char *) ssh_keys.enc_name, converted, client_server);

            PyObject_CallObject(pFunc1, pValue1);
            pResult = PyObject_CallObject(pFunc2, pValue2);
            ...
            if (temp_bytes != NULL) {
                ...
                if (strlen(my_result) != 0) {
                    ssh_decrypted_data.data = my_result;
                    ssh_decrypted_data.data_len = (int)strlen(my_result);
                    guint8 content_type = tvb_get_guint8(tvb, 0);
                    guint8 curr_layer_num_ssh = pinfo->curr_layer_num;
                    ssh_save_decrypted_record(pinfo, tvb_raw_offset(tvb)+offset, content_type, curr_layer_num_ssh);
                }
            } else {
                return -1;
            }
        ...
        decrypted = ssh_get_record_info(tvb, proto_ssh, pinfo, tvb_raw_offset(tvb)+offset, pinfo->curr_layer_num, &record);
        if (decrypted) {
            add_new_data_source(pinfo, decrypted, "Decrypted SSH");
        }
...
proto_register_ssh(void) {
    ...
    //TODO next lines are new
    prefs_register_filename_preference(ssh_module, "keyd", "Client to Server Key",
                                       "Key Client to Server HEX string. Should be 0 to 16 bytes.",
                                       &ssh_keys.key_client_to_server, FALSE);

    prefs_register_filename_preference(ssh_module, "keyc", "Server to Client Key",
                                       "Key server to client as HEX string. Should be 0 to 16 bytes.",
                                       &ssh_keys.key_server_to_client, FALSE);

    prefs_register_filename_preference(ssh_module, "keya", "Client to Server IV",
                                       "Iv Client to server as HEX string. Should be 0 to 16 bytes.",
                                       &ssh_keys.iv_client_to_server, FALSE);

    prefs_register_filename_preference(ssh_module, "keyb", "Server to Client IV",
                                       "IV server to client as HEX string. Should be 0 to 16 bytes.",
                                       &ssh_keys.iv_server_to_client, FALSE);

    prefs_register_string_preference(ssh_module, "enc_name", "Encryption Cipher Name",
